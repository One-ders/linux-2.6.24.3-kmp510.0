...
#define AR6002_NORMAL_CLOCK_RATE 0x016e3600
...
void
HcdCommonBufferCopy(UINT8         DataSize,
                         PVOID         pDest,
                         PVOID         pSrc,
                         INT           Bytes,
                         BOOL          ByteSwap) {
...
static
SDIO_STATUS DoDataFrames(PSDHCD_DEVICE pDevice,
                                PVOID         pBuffer,
                                INT           Bytes,
                                ATH_TRANS_CMD Cmd,
                                BOOL          ByteSwap) {
	...
	do {
		...
		if (ATH_IS_TRANS_PIO(pReq)) {
			...
			if (ATH_IS_TRANS_PIO_INTERNAL(pReq)) {
				...
				/* external reads are more complicated, DONE */
		...
		if (ATH_IS_TRANS_READ(pReq)) { //DONE
...
SDIO_STATUS HcdInitialize(PSDHCD_DEVICE pDevice) {
	...
	do {
		...
		pDevice->SpiConfigShadow |= ATH_SPI_CONFIG_KEEP_AWAKE | ATH_SPI_CONFIG_KEEP_AWAKE_ON_INTR;

		/* delay a bit before our first SPI operation */
		OSSleep(pDevice->PowerUpDelay);

		status = DoPioWriteInternal(pDevice,ATH_SPI_CONFIG_REG,0x8000);
		...
		DoPioReadInternal(pDevice,ATH_SPI_CONFIG_REG, &status);
