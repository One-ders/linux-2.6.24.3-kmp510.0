...
#define __FILE__ "/app1/colman2/ar6k/AR6102-2.2.0.78/AR6kSDK.build_sw.78/host/os/linux/ar6000_drv.c"
...
void stack_force_interrupt(void);
void get_reg_code_from_file(char *regfile, A_UINT32 *reg_code);
...
char *softmac = "/system/wifi/softmac";
...
ar6000_set_host_app_area(AR_SOFTC_T *ar)
{
    ...
    address = TARG_VTOP1(ar->arTargetType, data);
...
//    printk("debug_hdr_ptr: 0x%x\n", debug_hdr_ptr);
    ...
    if (debug_hdr_ptr) {
        address = TARG_VTOP1(ar->arTargetType, debug_hdr_ptr);
        length = sizeof(struct dbglog_hdr_s);
        ar6000_ReadDataDiag(ar->arHifDevice, address,
                            (A_UCHAR *)&debug_hdr, length);
        address = TARG_VTOP1(ar->arTargetType, (A_UINT32)debug_hdr.dbuf);
        firstbuf = address;
        ...
        do {
            address = TARG_VTOP1(ar->arTargetType, (A_UINT32)debug_buf.buffer);
            ...
            address = TARG_VTOP1(ar->arTargetType, (A_UINT32)debug_buf.next);
            length = sizeof(struct dbglog_buf_s);
            if(A_OK != ar6000_ReadDataDiag(ar->arHifDevice, address,
                                (A_UCHAR *)&debug_buf, length))
            {
...
aptcTimerHandler(unsigned long arg)
{
    ...
    ar = (AR_SOFTC_T *)arg;
    ATH_ASSERT(ar != NULL);
    ATH_ASSERT(!timer_pending(&aptcTimer));
    ...
    if (throughput < APTC_LOWER_THROUGHPUT_THRESHOLD) {
        ...
        ATH_ASSERT(ar->arWmiReady == TRUE);
        AR6000_SPIN_UNLOCK(&ar->arLock, 0);
        status = wmi_powermode_cmd(ar->arWmi, REC_POWER);
        AR6000_SPIN_LOCK(&ar->arLock, 0);
        ATH_ASSERT(status == A_OK);
...
/* ATHENV */
//#ifdef ANDROID_ENV
extern void eeprom_ar6000_transfer(HIF_DEVICE *device, char *eeprom_data, char *softmac, A_UINT32 regCode);
//#endif
/* ATHENV */
...
//#ifdef ANDROID_ENV
	...
	if (reg_file) {
        ...
        eeprom_ar6000_transfer(ar->arHifDevice, eeprom_data, softmac, regCode);
#else
...
void get_reg_code_from_file(char *regfile, A_UINT32 *reg_code) {
	...
	oldfs=get_fs();
	set_fs(KERNEL_DS);
	filp=filp_open(regfile, O_RDONLY, S_IRUSR);
	if (IS_ERR(filp)) {
		set_fs(oldfs);
		return;
	}
...
//                set_fs(oldfs);
	...
	length=i_size_read(inode->i_mapping->host);
        if (length>1) {
        	...
        	if (filp->f_op->read(filp, reg_code, 2, &filp->f_pos)==2) {
			filp_close(filp,NULL);
fot:
//goto err;
			set_fs(oldfs);
...
int ar6000_init(struct net_device *dev)
{
    ...
    do {
        ...
        ATH_ASSERT(arAc2EndpointID(ar,WMM_AC_BE) != 0);
        ATH_ASSERT(arAc2EndpointID(ar,WMM_AC_BK) != 0);
        ATH_ASSERT(arAc2EndpointID(ar,WMM_AC_VI) != 0);
        ATH_ASSERT(arAc2EndpointID(ar,WMM_AC_VO) != 0);
    ...
    printk("Youli::%s: wait wmi_ready event to trigger host app area\n\n",__func__);
...
ar6000_ibss_map_epid(struct sk_buff *skb, struct net_device *dev, A_UINT32 * mapNo)
{
    ...
    if (eptMap == -1) {
        eptMap = ar->arNodeNum;
        ar->arNodeNum ++;
        ATH_ASSERT(ar->arNodeNum <= MAX_NODE_NUM);
    }
...
ar6000_tx_complete(void *Context, HTC_PACKET *pPacket)
{
    ...
    ATH_ASSERT(skb);
    ATH_ASSERT(pPacket->pBuffer == A_NETBUF_DATA(skb));

    if (A_SUCCESS(status)) {
        ATH_ASSERT(pPacket->ActualLength == A_NETBUF_LEN(skb));
    }
...
ar6000_ready_event(void *devt, A_UINT8 *datap, A_UINT8 phyCap, A_UINT32 vers)
{
    ...
    ar->arWmiReady = TRUE;
    wake_up(&arEvent);
    printk("Youli::%s: use wmi_ready event trigger setting host app\n", __FUNCTION__);
    if (ar6000_set_host_app_area(devt)!=0) {
    	printk("Youli::%s:#########Unable to set the host app area\n", __FUNCTION__);
    }
...
{
    ATH_ASSERT(num <= (HTC_MAILBOX_NUM_MAX - 1));
                        ...
                        A_UINT8 assocRespLen, A_UINT8 *assocInfo, A_UINT16 protocolReasonStatus)
{
    ...
    if (ar->arNetworkType&INFRA_NETWORK) {
        ...
        wireless_send_event(ar->arNetDev, SIOCGIWAP, &wrqu, NULL);
//	A_MEMZERO(xx, 16);
//	wireless_send_event(*s5,0x8b15,s0,0);
    }
}
...
/* note: IWEVCUSTOM only exists in wireless extensions after version 15 */
	...
	if (size > IW_CUSTOM_MAX) {
		if (print_wmi_message_too_big<5) {
			print_wmi_message_too_big++;
...
void ar6000_dtimexpiry_event(AR_SOFTC_T *ar)
{
    ...
    ATH_ASSERT(isMcastQueued == FALSE);
